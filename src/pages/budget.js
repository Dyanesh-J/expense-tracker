      ...
      <div class="card mb-3">
        ...
        <div class="form-row" style="align-items:flex-end;">
          <div class="form-group" style="margin-bottom:0;">
            <label class="form-label">Budget Amount (Rs)</label>
...
function renderOverview(container, monthly) {
  ...
  const isOverBudget = monthly.remaining < 0;
  const remainingLabel = isOverBudget ? 'Over By' : 'Remaining';
  const remainingText = isOverBudget
    ? `-${formatCurrency(Math.abs(monthly.remaining))}`
  ...
  el.innerHTML = `
    <div class="stats-grid" style="grid-template-columns: repeat(3, 1fr);">
      <div class="stat-card" style="--accent-color:var(--green);">
        ...
        <div class="stat-value" id="bov-budget">Rs0</div>
      ...
      <div class="stat-card" style="--accent-color:var(--orange);">
        ...
        <div class="stat-value" id="bov-spent">Rs0</div>
      ...
      <div class="stat-card" style="--accent-color:${status === 'safe' ? 'var(--green)' : status === 'warning' ? 'var(--yellow)' : 'var(--pink)'}">
        ...
        <div class="stat-value" id="bov-remaining" style="color:${status === 'danger' ? 'var(--pink)' : 'var(--green)'}">${remainingText}</div>
        <div class="stat-sub">${pct.toFixed(1)}% used</div>
        <div class="stat-icon">${isOverBudget ? 'Alert' : 'Safe'}</div>
      </div>
    </div>
    <div class="card" style="margin-top:0; padding:20px 24px;">
      <div class="flex-between" style="margin-bottom:10px;">
        ...
        <span class="stat-badge ${status === 'safe' ? 'green' : status === 'warning' ? 'orange' : 'pink'}">
          ${pct >= 100 ? 'Over Budget' : pct >= 80 ? 'Almost There' : 'On Track'}
        </span>
      </div>
...
function renderCatBreakdown(container, catData, monthTotal) {
  ...
  const entries = Object.entries(catData).filter(([, value]) => value > 0).sort((a, b) => b[1] - a[1]);

  if (!entries.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon">0</div><p>No expenses this month</p></div>`;
    return;
  }
...
function renderAllBudgets(container, budgets, monthly) {
  ...
  const rows = [...allMonths].sort().reverse().map((month) => {
    ...
    const status = pct >= 100 ? 'danger' : pct >= 80 ? 'warning' : 'safe';
    return { month, budget, spent, remaining, pct, status };
  });

  if (!rows.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon">0</div><p>No data yet</p></div>`;
    return;
  }
        ...
        ${rows.map((r) => `
          <tr>
            ...
            <td style="font-family:var(--font-mono); color:${r.pct >= 100 ? 'var(--pink)' : 'var(--green)'}">
              ${r.budget ? (r.remaining < 0 ? `-${formatCurrency(Math.abs(r.remaining))}` : formatCurrency(r.remaining)) : '-'}
            </td>
            <td>
              ${r.budget ? `
